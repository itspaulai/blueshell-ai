...
export function ChatContainer({ conversationId, onConversationCreated }: ChatContainerProps) {
  ...
  const handleSendMessage = async (content: string) => {
    ...
    try {
      ...
      // Update conversation with final bot message
      const updatedMessages = messages.concat([userMessage, { ...initialBotMessage, content: fullMessage }]);
    ...
    const loadConversation = async () => {
      if (conversationId) {
        ...
        if (conversation && conversation.messages.length > 0) {
          ...
          setMessages([]); // Start with an empty chat for new conversations
        }
      } else {
  ...
  useEffect(() => {
    if (conversationId && messages.length > 0) {
      chatDB.updateConversation(conversationId, messages, undefined, true);
    }
    scrollToBottom();
        ...
        <div className="max-w-3xl mx-auto py-6">
          {messages.length === 0 && (
            <div className="flex justify-center items-center h-[calc(100vh-200px)]">
              <span className="text-4xl font-bold text-blue-500">Start a new chat</span>
            </div>
          )}
